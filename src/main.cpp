...
// minimal) embedded test project - box, line, and label objects, the
// first slice of the M5 Dial DDF's supportedObjectTypes to be ported
// (MQTTIconField/MqttDataField/level-indicator/icon/SoftwareButton still
// TODO, see ColorScreenRenderer.h). Confirms the full pipeline (LittleFS
// file -> ProjectLoader JSON parsing -> ColorScreenRenderer -> canvas ->
// blit) end to end on real hardware before wiring in MQTT/deploy.
#include <M5Dial.h>
#include <LittleFS.h>
...
void writeTestProject() {
  // Always overwrites (not "if missing") - this is a bring-up fixture that
  // changes as object types get ported, not real persisted data; an
  // "if missing" guard bit us once already (a fixed test_project.h change
  // silently had no effect because the stale LittleFS copy from a
  // previous flash was never replaced).
